...
finaltable_values <-vData_refID_WAPu_NA_N
...
rm(a,b)
ctrl <- trainControl(method = "repeatedcv", repeats = 3,verboseIter = F) #crossvalidation 3 razy powtórzona | number = (defaults to 10)
...
for (B in c("X0","X1","X2")) {
  ...
  vData_B <- vData_refID_WAPu_NA_N %>% filter(BuilPRED == B)
  ...
  for (i in 1:2) {
    ...
    FLLfit1KNN <- train(
      ...
      #tuneLength = 6,
      tuneGrid = expand.grid(k = c(5:10)),
      trControl = ctrl
    )
    name_mod <- paste("CORD",th,B,column[i], sep = "_")
...
for (model in model_list) {
  ...
  name_table <- paste0("FinnalTable",id,substr(model, 13, 25))
  table <- cbind(finaltable_values %>% filter(BuilPRED==id),pred_loc) 
  assign(name_table,table)
  saveRDS(assign(name_table,table),paste0("Data/Results/",name_table,".RDS"))
...
name_model <- "knn_50_nFLOOR_2"

assign(name_model, data.frame(compare.model))
knn_50_nFLOOR_2$model <- name_model
knn_50_nFLOOR_2
CORD_table <- readRDS("Data/Results/CORD_table.RDS")
CORD_table <- rbind(CORD_table,knn_50_nFLOOR_2)
CORD_table 
# SAVE --------------------------------------------------------------------


saveRDS(knn_50_nFLOOR_2,file = paste0("Data/Results/",name_model ,".RDS"))
saveRDS(CORD_table,file = "Data/Results/CORD_table.RDS")
write.csv(CORD_table,"csv_outputs/CORD_table.csv")

# loc_pred_f(model = FLLfit1KNN_X1_LATITUDE_030_nFLOOR, test_data = validation_X1, LONGLAT = "LATITUDE")
...
F_save_models_ls <- function(objects,object_names) {
  ...
  sapply(1:length(objects), function(i) {  ## co to za cuda??
    filename = paste0("data/Results/models/", object_names[i], ".rds")
...
model_list <-  ls() %>% vars_select(starts_with("CORD")) %>%  
  vars_select(ends_with("TUDE")) %>% 
  as_tibble()
model_list_name <- unname(ls() %>% vars_select(starts_with("CORD")) %>%  
                            vars_select(ends_with("TUDE")))
